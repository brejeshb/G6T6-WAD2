<template>
    <div id="app">
      ...
      <div class="row">
        <div class="container col-lg-6 col-12 my-3">
          ...
          <div class="forest">
            <table
              style="box-shadow: 5px 5px 10px 2px rgb(0 0 0 / 0.8); background-color: brown;"
              ...
              <tr v-for="i in size" :key="i">
                <td v-for="j in size" :key="j">
                  <div v-if="(i - 1) * size + j - 1 < items().length" class="box">
                    <img
                      @click="facts(calculation.name_of_trees_planted[(i - 1) * size + j - 1])"
  ...
  function purchase(item) {
    if (cal_data.value.current_tree !== '') {
      ...
      cal_data.value.current_tree = `${item.current_tree}.png`;
    }
  }
  ...
  const calculation = computed(() => {
    ...
    let required_point = trees.value[jsondata.current_tree];
    ...
    jsondata.trees_planted.forEach(thing => {
      let two = thing.split("_").map(i => i[0]).join('');
      cal_data.value.trees_planted.push(thing.includes("tree") ? 
        `./src/views/shop_page/img/${thing}/${two}5.png` : `./src/views/shop_page/img/${thing}/${two}6.png`);
    });
  
    cal_data.value.current_points = jsondata.point;
    let counter = jsondata.current_tree.includes("tree") ? 
      Math.floor(cal_data.value.percent / (100 / 5)) + 1 : 
      Math.floor(cal_data.value.percent / (100 / 6)) + 1;
  
    let name = cal_data.value.current_tree.split("_").map(i => i[0]).join('');
    cal_data.value.picture = `./src/views/shop_page/img/${cal_data.value.current_tree}/${name}${counter}.png`;
  
    return cal_data.value;
  });
  </script>
...
.item-grid {
    ...
    max-height:85%;  /* Adjust based on your needs */
