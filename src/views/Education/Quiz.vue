  ...
  background-size: 100vw auto;height: 100vh;">
    <section class="py-5 text-center top_section" data-aos="fade-up">
      <div class="row py-lg-5">
        <div class="col-lg-6 col-md-8 mx-auto w-100 d-flex justify-content-center">
          ...
          <div v-if="!isQuizActive">
            ...
            <div class="row">
              <div class="col-xl-3 col-lg-4 col-md-6 mb-4" v-for="quiz in filteredQuizzes" :key="quiz.id"
                ...
                <div class="card">
                  <div class="card-body">
                    <h5 class="card-title" style="color:#f8f4ec">{{ quiz.category.charAt(0).toUpperCase() +
                      quiz.category.slice(1) }} Quiz</h5>
                    <p class="card-text" style="color:#f8f4ec">{{ quiz.title }}</p>
                    <div class="mb-3">
                      <p class=" mb-1" style="color:#f8f4ec">
                        Attempts: {{ quizAttempts[quiz.id].attempts }} | Last Score: {{ quizAttempts[quiz.id].score
                        }}/{{ quiz.questions.length }}
                      </p>
                      <div class="progress">
                        <div class="progress-bar" :style="{ width: getScorePercentage(quiz.id) + '%' }"
                          role="progressbar" :aria-valuenow="getScorePercentage(quiz.id)" aria-valuemin="0"
                          aria-valuemax="100"></div>
                      </div>

                    </div>
                    <button class="btn btn-primary mt-2" @click="startQuiz(quiz)"
                      v-if="!quizAttempts[quiz.id]?.completed">Start Quiz</button>
                    <span class="d-inline-block" v-else data-bs-toggle="tooltip" data-bs-placement="top" tabindex="0"
                      data-bs-custom-class="custom-tooltip"
                      data-bs-title="You can no longer gain points from completing this quiz until next month">
                      <button class="btn btn-primary mt-2" @click="startQuiz(quiz)"
                        :disabled="quizAttempts[quiz.id] && quizAttempts[quiz.id].completed">Start Quiz</button></span>
                  </div>
                </div>
              </div>
            </div>
